...
function addText(type, id, text, fontSize, top, left, isTemplate) {
  ...
  box.draggable=true;
  box.dataset.type=type;
  box.innerText=(typeof(text)=='undefined')?language.templates.text:text.replaceAll("\\n","\n");
  if (isTemplate) {
    document.getElementById("templates_area").appendChild(box);
    box.ondragstart=dragTemplate;
  } else {
...
templates.push({
  ...
  maxEdgesIn: 0,
  maxEdgesOut: 1,
  addFunc: (id, nr, top, left, setup, isTemplate, elements)=>addBox("SignalSource",id,language.templates.signalSource+"<br>"+nr,language.templates.signalSourceHint,"#FB3","#1C1",top,left,isTemplate),
  setup: {signal: '', b: 1}
});
templates.push({
  type: 'Vertex',
